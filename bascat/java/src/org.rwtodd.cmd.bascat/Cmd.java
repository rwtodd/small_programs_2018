...
public class Cmd {
   ...
   private boolean nextToken(StringBuilder sb) {
      ...
      int nxt = in.readu8();
      if (nxt >= 0xFD) { nxt = (nxt << 8) | in.readu8(); }
       
      // first check for cases to clean up sequences...
      if (nxt == 0x3A && in.peek(0xA1)) {
          sb.append("ELSE"); in.skip(1); 
      } else if (nxt == 0x3A && in.peek2(0x8F,0xD9)) {
          sb.append('\''); in.skip(2); 
      } else if (nxt == 0xB1 && in.peek(0xE9)) {
          sb.append("WHILE"); in.skip(1); 
      }

      // a couple of ranges we can check first...
      else if (nxt >= 0x20 && nxt <= 0x7E) { sb.append((char)nxt); }
      
      //  a bunch of ranges that are predefined tokens...
      else if (nxt >= 0x11 && nxt <= 0x1B) { sb.append(Tokens[nxt-0x11]); } 
      else if (nxt >= 0x81 && nxt <= 0xF4) { sb.append(Tokens[nxt - 118]); }
      else if (nxt >= 0xFD81 && nxt <= 0xFD8B) { sb.append(Tokens[nxt - 64770]); }
      else if (nxt >= 0xFE81 && nxt <= 0xFEA8) { sb.append(Tokens[nxt - 65015]); }
      else if (nxt >= 0xFF81 && nxt <= 0xFFA5) { sb.append(Tokens[nxt - 65231]); }

      // a few special cases, mostly for number fomatting
      else switch(nxt)  {

      case 0: hasMore = false; break;
      case 0x0B: sb.append(String.format("&O%o", in.read16())); break; // OCTAL
      case 0x0C: sb.append(String.format("&H%X", in.read16())); break; // HEX
      case 0x0E: sb.append(in.readu16()); break;                       // DECIMAL UNSIGNED SHORT
      case 0x0F: sb.append(in.readu8()); break;                        // DECIMAL UNSIGNED BYTE
      case 0x1C: sb.append(in.read16()); break;                        // DECIMAL SIGNED SHORT
      case 0x1D: sb.append(String.format("%g",in.readf32())); break;   // FLOAT 32
      case 0x1F: sb.append(String.format("%g",in.readf64())); break;   // FLOAT 64

      default:   sb.append(String.format("<UNK! %x>", nxt)); break;
      }
   ...
   private void cat(PrintStream ps) {
       ...
       while(!in.atEOF()) {
           if(in.readu16() == 0) break; // 0 pointer == EOF
           sb.append(in.readu16()); 
           sb.append("  ");
           while(nextToken(sb)) { /* do nothing */ }
    ...
    private Cmd(byte[] source) {
        ...
        switch(in.readu8()) {
          case 0xFE: Unprotector.unprotect(source);
                     break;
          case 0xFF: break;
          default:
